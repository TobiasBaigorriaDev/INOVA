...
function Collections({ toggleFavorite, favorites }) {
    ...
    // NUEVO: Estado para manejar la animación del ícono
    ...
    // MODIFICADO: Lógica de alternancia con animación
    const toggleSortOrder = () => {
        // 1. Iniciamos la animación (el ícono se desvanece y se achica)
        setIsAnimatingSort(true);

        // 2. Esperamos a que termine de desaparecer (200ms) para cambiar el orden
        setTimeout(() => {
            let nextOrder = '';
            if (sortOrder === '') {
                nextOrder = 'asc';
            ...
            // 3. Volvemos a aparecer el nuevo ícono con efecto de "emergencia"
            setTimeout(() => {
                setIsAnimatingSort(false);
            }, 50);
                    ...
                    <div style={{
                        ...
                        <button
                            ...
                            style={{
                                ...
                                fontSize: '12px',
                                letterSpacing: '2px',
                                color: sortOrder ? 'var(--primary)' : 'var(--text-muted)',
                                fontWeight: sortOrder ? 'bold' : 'normal',
                                padding: 0
                            }}
                        >
                            Precio
                            {/* NUEVO: Contenedor con estilos dinámicos para la animación */}
                            <span style={{
                                display: 'flex',
                                alignItems: 'center',
                                opacity: isAnimatingSort ? 0 : 1,
                                transform: isAnimatingSort ? 'scale(0.5)' : 'scale(1)',
